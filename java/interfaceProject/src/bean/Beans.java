/*
 * 자바가 실행이 되면 사용할 모든 객체를 생성해서 저장하고 있는 객체 
 * static{}에서 모든 객체를 생성 
 * Map<String, Service> serviceMap : 생성도니 객체를 저장하는 변수
 * 호출해서 사용하려는 곳에서 해당되는 service를 받아가는 메서드 : getService()
 * 저장되어 있는 객체들은 service()를 이용해서 실행 할 수 있는 형태로 만들어 준다. 
...
public class Beans {
	...
	//클래스에 맨처음 한번만  실행 되는 초기화 블록 -> 모든 객체는 여기서만 생성을 한다. 
	static {
		System.out.println("Beans.static{} : 모든 서비스시 객체생성");
	...
	public static Service getService(String key) {
		// 저장 되어있는 서비스 중 key에 해당이 되는 서비스를 찾아서 넘겨준다. 
