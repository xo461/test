...
public class NoticeView {
	...
	public void list(List<NoticeDTO> list) {
		//타이틀 출력
		Output.list("공 지 사 항   리 스 트");
		System.out.println("| 글번호  | 제목  | 내용  | 시작일  | 마감일  | 조회수 |");
...
//		 "NoticeDTO [no=" + no + ", title=" + title + ", content=" + content + ", startDate=" + startDate
	//		+ ", endDate=" + endDate + ", writeDate=" + writeDate + ", updateDate=" + updateDate + "]";
		...
		for(NoticeDTO dto : list) {
			System.out.printf("|  %2d  |  %s  |  %s  |  %s  |  %s  |  %2d  |%n",
					dto.getNo(), dto.getTitle(), dto.getContent(), dto.getStartDate(), 
					dto.getEndDate(), dto.getHit());
		}
		System.out.println("\n");
	}
