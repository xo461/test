...
//    	2. 정렬해서 가져온 데이터에 순서 번호을 붙인다. (페이지 처리 )
    	sql = " select rownum rnum, no, title, startDate, endDate, "
    			+ " writeDate, updateDate from (" + sql + ")";
    			
//    	3. 순서번호를 붙인 데이터에서 페이지에 맞는 데이터를 찾아온다. (페이지처리 )
    	sql = " select * from("+sql+") "
 ...
 // servico.NoticeService -> dao.NoticeDAO
    ...
    public int getTotalRow() throws Exception{
    	...
    	System.out.println("NoticeDAO.getTotalRow().totlaRow"+totalRow);
