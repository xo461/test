...
public class MagazineListService implements Service{
	
	private MagazineDAO dao;
	
	public MagazineListService(MagazineDAO dao) {
		this.dao = dao;
	}
@Override
	public List<MagazineDTO> service(Object[] objs) throws Exception{
		...
		// 데이터 처리 부문에 해당된다.
		System.out.println("MagazineListService.service()");
		...
		System.out.println("MagazineListService.service().pageObject:"+pageObject);
